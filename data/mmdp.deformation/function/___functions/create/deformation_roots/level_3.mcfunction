execute align xyz positioned ~0.5 ~0.5 ~0.5 if block ~1 ~ ~ #mmdp.deformation:deformable_blocks run summon item_display ~ ~ ~ {\
	...
	item_display:"none",item:{id:"minecraft:jigsaw",count:1,components:{"minecraft:item_model":"mmdp.deformation:deformation_roots","minecraft:custom_model_data":3}},\
...
execute as @e[type=item_display,tag=mmdp.deformation_roots.new,distance=..1] if predicate mmdp.deformation:chance/0.3 run data modify entity @s item.components."minecraft:custom_model_data" set value 4
